...
class AsyncAuthServiceProxy:
    ...
    def __init__(self, service_url, service_name=None, reconnect_timeout=15, reconnect_amount=2, request_timeout=90):
        ...
        if AsyncAuthServiceProxy._client is None:
            AsyncAuthServiceProxy._client = AsyncClient(timeout=Timeout(request_timeout), limits=Limits(max_connections=200, max_keepalive_connections=10))
...
async def create_new_pastelid(rpc_connection, passphrase):
...
async def get_recent_transactions_func(rpc_connection, blocks=5):
    recent_txids = []
    best_block_hash = await rpc_connection.getbestblockhash()
    best_block_details = await rpc_connection.getblock(best_block_hash)
    current_block_height = best_block_details['height']
...
async def benchmark_rpc_calls(rpc_connection, pastelid, passphrase):
    async def rpc_call():
        message_to_sign = "some_message"
        return await sign_message_with_pastelid_func(rpc_connection, pastelid, message_to_sign, passphrase)

    async def block_height_call():
        return await get_current_pastel_block_height_func(rpc_connection)

    async def masternode_call():
        return await check_masternode_top_func(rpc_connection)

    async def recent_tx_call():
        txids = await get_recent_transactions_func(rpc_connection)
        return await asyncio.gather(*[get_raw_transaction_func(rpc_connection, txid) for txid in txids])
    ...
    with open("rpc_benchmark_results.txt", "a") as f:
        while True:
            ...
            try:
                ...
                timestamp = datetime.now(timezone.utc).isoformat()
                f.write(f"{timestamp}: Failed at {max_concurrent_calls} concurrent RPC calls. Error: {e}\n")
                logger.error(f"Failed at {max_concurrent_calls} concurrent RPC calls. Error: {e}")
                break

        f.write(f"{timestamp}: Maximum successful concurrent RPC calls: {max_successful_calls}\n")
        logger.info(f"Maximum successful concurrent RPC calls: {max_successful_calls}")

async def main():
    rpc_host, rpc_port, rpc_user, rpc_password, other_flags = get_local_rpc_settings_func("/home/akobrin/.pastel-devnet")
    rpc_connection = AsyncAuthServiceProxy(f"http://{rpc_user}:{rpc_password}@{rpc_host}:{rpc_port}")

    passphrase = "your_passphrase_here"
    pastelid = await create_new_pastelid(rpc_connection, passphrase)

    try:
        await benchmark_rpc_calls(rpc_connection, pastelid, passphrase)
    finally:
        await AsyncAuthServiceProxy.close_client()
